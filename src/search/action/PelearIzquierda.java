...
public class PelearIzquierda extends SearchAction {
	...
	public SearchBasedAgentState execute(SearchBasedAgentState s) {
		...
		if (planta.getZombies().containsKey(punto) && planta.getZombies().get(punto).getEnergia() < energia) {
	...
	public EnvironmentState execute(AgentState ast, EnvironmentState est) {
		...
		// matarlo
		// entonces puede pelear
		if (ambiente.getZombies().containsKey(punto) && ambiente.getZombies().get(punto).getEnergia() < energia) {
			...
			planta.setEnergia(energia - ambiente.getZombies().get(punto).getEnergia());

			planta.setCantidadZombies(ambiente.getCantidadZombies() - 1);
			planta.getZombies().remove(punto);
